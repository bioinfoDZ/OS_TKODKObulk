...
for(filedex in 1:length(files)) {
  ...
  dat <- as.data.frame(read.table(file, header = T)[,5])
  colnames(dat) <- samp
...
rm(mart,pb,rownamesdf,genes,files,samps,samp,total,file,filedex)
...
saveRDS(l, 'data/parsed/parsed.rds')
